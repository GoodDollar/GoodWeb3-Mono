...
function prepareTscCommands(files) {
  ...
  for (const file of files) {
    ...
    if (!(package in toCompile)) {
      ...
      if (package === "sdk-v2") {
        toCompile[package].push("packages/sdk-v2/src/custom.d.ts");
      }
    }
...
module.exports = {
  "packages/*/src/**/*.{js,jsx,ts,tsx, d.ts}": async files => {
    ...
    return [`eslint --no-ignore --max-warnings=0 --fix ${filtered.map(path => `"${path}"`).join(" ")}`, ...tscCommands];
  }
};
